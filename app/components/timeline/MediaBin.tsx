...
import { FileVideo, FileImage, Type, Clock, Upload, Music, Trash2, SplitSquareHorizontal, ChevronDown, ChevronUp, List, Layers, ArrowUpDown, Play, Pause, Volume2, VolumeX } from "lucide-react";
...
const AudioPreview = ({ src }: { src: string }) => {
  ...
  const onPointerDownTrack = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    e.preventDefault();
  ...
  return (
    <div className="w-full flex items-center gap-2 select-none">
      ...
      <div ref={trackRef} onPointerDown={onPointerDownTrack} className="relative w-full h-0.5 rounded cursor-pointer bg-black/25 dark:bg-white/25">
        <div
...
export default function MediaBin() {
  ...
  const [collapsed, setCollapsed] = useState<{ [key in "videos" | "gifs" | "images" | "audio" | "text"]: boolean }>({
    videos: false,
    gifs: false,
    images: false,
    audio: false,
          ...
          <div className="flex items-center gap-1.5">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-5 w-5 p-0 text-muted-foreground/70 hover:text-foreground bg-transparent hover:bg-transparent" title="Sort">
              ...
              <DropdownMenuContent align="end" className="min-w-[12rem]">
                ...
                <DropdownMenuItem onClick={() => setSortBy('default')} className={`text-[12px] gap-2 ${sortBy === 'default' ? 'text-primary' : ''}`} data-variant="ghost">
                  <ArrowUpDown className={`h-3 w-3 ${sortBy === 'default' ? 'text-primary' : ''}`} /> Original order
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy('name_asc')} className={`text-[12px] gap-2 ${sortBy === 'name_asc' ? 'text-primary' : ''}`} data-variant="ghost">
                  <ChevronUp className={`h-3 w-3 ${sortBy === 'name_asc' ? 'text-primary' : ''}`} /> Name A–Z
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy('name_desc')} className={`text-[12px] gap-2 ${sortBy === 'name_desc' ? 'text-primary' : ''}`} data-variant="ghost">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1 panel-scrollbar">
        ...
        {arrangeMode === 'group' && (
          <div className="space-y-2">
            {([
              { key: 'videos' as const, title: 'Videos', items: groupedItems.videos, count: counts.videos },
              { key: 'gifs' as const, title: 'GIFs', items: groupedItems.gifs, count: counts.gifs },
              { key: 'images' as const, title: 'Images', items: groupedItems.images, count: counts.images },
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="w-full max-w-[760px] max-h-[80vh] border border-border rounded-md bg-popover shadow-lg flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between px-3 py-2 border-b border-border/50">
                ...
                <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={closePreview}>Close</Button>
              </div>
              <div className="p-3 flex items-center justify-center overflow-auto">
                {previewItem.mediaType === 'video' && (
                  <video src={(previewItem.mediaUrlLocal || previewItem.mediaUrlRemote) || undefined} controls className="max-w-full max-h-[60vh] rounded" />
                )}
                {previewItem.mediaType === 'image' && (
                  <img src={(previewItem.mediaUrlLocal || previewItem.mediaUrlRemote) || undefined} alt={previewItem.name} className="max-w-full max-h-[60vh] rounded object-contain border border-border/50" />
                )}
